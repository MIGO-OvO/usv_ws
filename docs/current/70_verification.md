...
| COMMAND_LONG | 对照 `mavlink_trigger_node.py`、`usv_mavlink_router_bridge.py` | `31010..31019` 一致 |
...
| QGC 命令 | QGC 发送 `COMMAND_LONG 31010` | `/usv/mavlink_cmd_rx` 收到，trigger 发布状态，ACK 返回 |
...
| 步骤 | 观测点 | 通过标准 |
|---|---|---|
| 固件触发 | `NAV_SCRIPT_TIME` | ROS bridge 收到 `USV_SMPL` |
| ROS 采样 | `/usv/trigger_status`、`/usv/pump_status` | 状态进入采样并执行序列 |
